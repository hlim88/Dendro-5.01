...
namespace ode
{
    namespace solver
    {
        ...
        RK4_CCZ4::RK4_CCZ4(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh): RK(pMesh,pTBegin,pTEnd,pTh)
        {
            ...
            m_uiStage=new double**[ccz4::CCZ4_RK4_STAGES];
            for(unsigned int stage=0;stage<ccz4::CCZ4_RK4_STAGES;stage++)
            {
        ...
        void RK4_CCZ4::applyInitialConditions(double** zipIn)
        {
        ...
        void RK4_CCZ4::reallocateMPIResources()
        {
            for(unsigned int index=0;index<ccz4::CCZ4_ASYNC_COMM_K;index++)
            {
                delete [] m_uiSendNodeBuf[index];
        ...
        void RK4_CCZ4::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices)
        {
        ...
        void RK4_CCZ4::performGhostExchangeVars(double** zipIn)
        {
        ...
        void RK4_CCZ4::intergridTransferVars(double**& zipIn, const ot::Mesh* pnewMesh)
        {
        ...
        void RK4_CCZ4::unzipVars(double** zipIn , double** uzipOut)
        {
        ...
        void RK4_CCZ4::unzipVars_async(double ** zipIn , double **uzipOut)
        {
        ...
        void RK4_CCZ4::zipVars(double** uzipIn , double** zipOut)
        {
        ...
        void RK4_CCZ4::applyBoundaryConditions()
        {

        }

        void RK4_CCZ4::performSingleIteration()
        {
            ...
            do
            {
                ...
                if(m_uiMesh->isActive())
                {
                    ...
                    for(unsigned int stage=0;stage<(ccz4::CCZ4_RK4_STAGES-1);stage++)
                    {
                    ...
                    current_t_adv=current_t+RK4_T[(ccz4::CCZ4_RK4_STAGES-1)]*m_uiT_h;


#ifdef DEBUG_RK_SOLVER
                    if(!rank)std::cout<<" stage: "<<(ccz4::CCZ4_RK4_STAGES-1)<<" begin: "<<std::endl;
                ...
                for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
...
#ifdef DEBUG_RK_SOLVER
                    if(!rank)std::cout<<" stage: "<<(ccz4::CCZ4_RK4_STAGES-1)<<" af rhs UNZIP RHS TEST:"<<std::endl;
                    for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        ot::test::isUnzipInternalNaN(m_uiMesh,m_uiUnzipVarRHS[index]);
#endif

                    zipVars(m_uiUnzipVarRHS,m_uiStage[(ccz4::CCZ4_RK4_STAGES-1)]);

                    /*for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                            m_uiStage[(ccz4::CCZ4_RK4_STAGES-1)][index][node]*=m_uiT_h;*/
                    ...
                    for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                    {
                        ...
                        for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        {
                            ...
                            for(unsigned int s=0;s<(ccz4::CCZ4_RK4_STAGES);s++)
                            {
        ...
        void RK4_CCZ4::storeCheckPoint(const char * fNamePrefix)
        {
        ...
        void RK4_CCZ4::restoreCheckPoint(const char * fNamePrefix,MPI_Comm comm)
        {
