...
namespace ode
{
    namespace solver
    {
        ...
        RK3_CCZ4::RK3_CCZ4(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh): RK(pMesh,pTBegin,pTEnd,pTh)
        {
            ...
            m_uiStage=new double**[ccz4::CCZ4_RK3_STAGES];
            for(unsigned int stage=0;stage<ccz4::CCZ4_RK3_STAGES;stage++)
            {
        ...
        void RK3_CCZ4::applyInitialConditions(double** zipIn)
        {
	    ...
	    double mp, mm, mp_adm, mm_adm, E, J1, J2, J3;
            ...
            for(unsigned int elem=m_uiMesh->getElementLocalBegin();elem<m_uiMesh->getElementLocalEnd();elem++)
            {
				...
				if(ccz4::CCZ4_ID_TYPE == 0) {
				   TwoPunctures((double)x,(double)y,(double)z,var,&mp,&mm,&mp_adm,&mm_adm,&E,&J1,&J2,&J3);
				}
        ...
        void RK3_CCZ4::reallocateMPIResources()
        {
            for(unsigned int index=0;index<ccz4::CCZ4_ASYNC_COMM_K;index++)
            {
                delete [] m_uiSendNodeBuf[index];
        ...
        void RK3_CCZ4::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices)
        {
        ...
        void RK3_CCZ4::performGhostExchangeVars(double** zipIn)
        {
        ...
        void RK3_CCZ4::intergridTransferVars(double**& zipIn, const ot::Mesh* pnewMesh)
        {
        ...
        void RK3_CCZ4::unzipVars(double** zipIn , double** uzipOut)
        {
        ...
        void RK3_CCZ4::unzipVars_async(double ** zipIn , double **uzipOut)
        {
        ...
        void RK3_CCZ4::zipVars(double** uzipIn , double** zipOut)
        {
        ...
        void RK3_CCZ4::applyBoundaryConditions()
        {

        }

        void RK3_CCZ4::performSingleIteration()
        {
            ...
            do
            {
                ...
                if(m_uiMesh->isActive())
                {
                    ...
                    for(unsigned int stage=0;stage<(ccz4::CCZ4_RK3_STAGES-1);stage++)
                    {
                    ...
                    for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        ...
                        for(unsigned int blk=0;blk<blkList.size();blk++)
                        {
                            ...
                            ccz4rhs_sep(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset, ptmin, ptmax, sz, bflag);
                    ...
                    current_t_adv=current_t+RK3_T[(ccz4::CCZ4_RK3_STAGES-1)]*m_uiT_h;


#ifdef DEBUG_RK_SOLVER
                    if(!rank)std::cout<<" stage: "<<(ccz4::CCZ4_RK3_STAGES-1)<<" begin: "<<std::endl;
                ...
                for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
...
#ifdef DEBUG_RK_SOLVER
                    if(!rank)std::cout<<" stage: "<<(ccz4::CCZ4_RK3_STAGES-1)<<" af rhs UNZIP RHS TEST:"<<std::endl;
                    for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        ot::test::isUnzipInternalNaN(m_uiMesh,m_uiUnzipVarRHS[index]);
#endif

                    zipVars(m_uiUnzipVarRHS,m_uiStage[(ccz4::CCZ4_RK3_STAGES-1)]);

                    /*for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                            m_uiStage[(ccz4::CCZ4_RK3_STAGES-1)][index][node]*=m_uiT_h;*/
                    ...
                    for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                    {
                        ...
                        for(unsigned int index=0;index<ccz4::CCZ4_NUM_VARS;index++)
                        {
                            ...
                            for(unsigned int s=0;s<(ccz4::CCZ4_RK3_STAGES);s++)
                            {
        ...
        void RK3_CCZ4::rkSolve()
        {
            ...
            const unsigned int refineNumVars=ccz4::CCZ4_NUM_REFINE_VARS;
            unsigned int refineVarIds[refineNumVars];
            for(unsigned int vIndex=0;vIndex<refineNumVars;vIndex++)
                refineVarIds[vIndex]=ccz4::CCZ4_REFINE_VARIABLE_INDICES[vIndex];
...
//                     char fPrefix[256];
//                     sprintf(fPrefix,"%s_wavelets_%d",ccz4::CCZ4_VTU_FILE_PREFIX.c_str(),m_uiCurrentStep);
//                     io::vtk::waveletsToVTU(m_uiMesh,(const double **)m_uiPrevVar,(const double**)m_uiUnzipVar,refineVarIds,refineNumVars,(const char*)fPrefix);
        ...
        void RK3_CCZ4::storeCheckPoint(const char * fNamePrefix)
        {
        ...
        void RK3_CCZ4::restoreCheckPoint(const char * fNamePrefix,MPI_Comm comm)
        {
