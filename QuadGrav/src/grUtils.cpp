...
namespace bssn
{
    ...
    void readParamFile(const char * fName,MPI_Comm comm)
    {
        ...
        par::Mpi_Bcast(&tpf_len,1,0,comm);
        ...
        if(!rank)
        {
           for(unsigned int k=0;k<vtu_len;k++)
            ...
            for(unsigned int k=0;k<tpf_len;k++)
                tpf_name[k]=TPID::FILE_PREFIX[k];
        ...
        MPI_Bcast(tpf_name,tpf_len+1,MPI_CHAR,0,comm);
        ...
        TPID::FILE_PREFIX=std::string(tpf_name);
    ...
    void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm)
    {
        ...
        if(rank==root)
        {
            ...
            sout<<YLW<<"\tBSSN_ELE_ORDER :"<<bssn::BSSN_ELE_ORDER<<NRM<<std::endl;
            ...
            sout<<YLW<<"\tBSSN_ENABLE_BLOCK_ADAPTIVITY :"<<bssn::BSSN_ENABLE_BLOCK_ADAPTIVITY<<NRM<<std::endl;
            ...
            sout<<YLW<<"\tBSSN_RK_TYPE :"<<bssn::BSSN_RK_TYPE<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_RK45_TIME_STEP_SIZE :"<<bssn::BSSN_RK45_TIME_STEP_SIZE<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_RK45_DESIRED_TOL :"<<bssn::BSSN_RK45_DESIRED_TOL<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_COMPD_MIN : ( :"<<bssn::BSSN_COMPD_MIN[0]<<" ,"<<bssn::BSSN_COMPD_MIN[1]<<","<<bssn::BSSN_COMPD_MIN[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_COMPD_MAX : ( :"<<bssn::BSSN_COMPD_MAX[0]<<" ,"<<bssn::BSSN_COMPD_MAX[1]<<","<<bssn::BSSN_COMPD_MAX[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_BLK_MIN : ( :"<<bssn::BSSN_BLK_MIN_X<<" ,"<<bssn::BSSN_BLK_MIN_Y<<","<<bssn::BSSN_BLK_MIN_Z<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_BLK_MAX : ( :"<<bssn::BSSN_BLK_MAX_X<<" ,"<<bssn::BSSN_BLK_MAX_Y<<","<<bssn::BSSN_BLK_MAX_Z<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_OCTREE_MIN : ( :"<<bssn::BSSN_OCTREE_MIN[0]<<" ,"<<bssn::BSSN_OCTREE_MIN[1]<<","<<bssn::BSSN_OCTREE_MIN[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_OCTREE_MAX : ( :"<<bssn::BSSN_OCTREE_MAX[0]<<" ,"<<bssn::BSSN_OCTREE_MAX[1]<<","<<bssn::BSSN_OCTREE_MAX[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tETA_CONST :"<<bssn::ETA_CONST<<NRM<<std::endl;
            sout<<YLW<<"\tETA_R0 :"<<bssn::ETA_R0<<NRM<<std::endl;
            sout<<YLW<<"\tETA_DAMPING :"<<bssn::ETA_DAMPING<<NRM<<std::endl;
            sout<<YLW<<"\tETA_DAMPING_EXP :"<<bssn::ETA_DAMPING_EXP<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_ETA_R0 :"<<bssn::BSSN_ETA_R0<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_ETA_POWER : ("<<bssn::BSSN_ETA_POWER[0]<<" ,"<<bssn::BSSN_ETA_POWER[1]<<" )"<<NRM<<std::endl;
            ...
            sout<<YLW<<"\tBH1 MASS :"<<bssn::BH1.getBHMass()<<NRM<<std::endl;
            sout<<YLW<<"\tBH1 POSITION (x,y,z) : ("<<bssn::BH1.getBHCoordX()<<", "<<bssn::BH1.getBHCoordY()<<", "<<bssn::BH1.getBHCoordZ()<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBH1 VELOCITY (x,y,z) : ("<<bssn::BH1.getVx()<<", "<<bssn::BH1.getVy()<<", "<<bssn::BH1.getVz()<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBH1 SPIN (||,theta,phi): ( "<<bssn::BH1.getBHSpin()<<", "<<bssn::BH1.getBHSpinTheta()<<", "<<bssn::BH1.getBHSpinPhi()<<" )"<<NRM<<std::endl;

            sout<<YLW<<"\tBH2 MASS :"<<bssn::BH2.getBHMass()<<NRM<<std::endl;
            sout<<YLW<<"\tBH2 POSITION (x,y,z) : ("<<bssn::BH2.getBHCoordX()<<", "<<bssn::BH2.getBHCoordY()<<", "<<bssn::BH2.getBHCoordZ()<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBH2 VELOCITY (x,y,z) : ("<<bssn::BH2.getVx()<<", "<<bssn::BH2.getVy()<<", "<<bssn::BH2.getVz()<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tBH2 SPIN (||,theta,phi): ( "<<bssn::BH2.getBHSpin()<<", "<<bssn::BH2.getBHSpinTheta()<<", "<<bssn::BH2.getBHSpinPhi()<<" )"<<NRM<<std::endl;
            ...
            sout<<YLW<<"\tBSSN_BH1_MAX_LEV:"<<bssn::BSSN_BH1_MAX_LEV<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_BH2_MAX_LEV:"<<bssn::BSSN_BH2_MAX_LEV<<NRM<<std::endl;
            sout<<YLW<<"\tBSSN_INIT_GRID_ITER:"<<bssn::BSSN_INIT_GRID_ITER<<NRM<<std::endl;
            

            #ifdef BSSN_REFINE_BASE_EH
                    sout<<YLW<<"\tBSSN_EH_REFINE_VAL  : "<<bssn::BSSN_EH_REFINE_VAL<<NRM<<std::endl;
                    sout<<YLW<<"\tBSSN_EH_COARSEN_VAL : "<<bssn::BSSN_EH_COARSEN_VAL<<NRM<<std::endl;
            #endif 

            sout<<YLW<<"\tBSSN_NUM_EVOL_VARS_VTU_OUTPUT :"<<bssn::BSSN_NUM_EVOL_VARS_VTU_OUTPUT<<NRM<<std::endl;
            ...
            sout<<bssn::BSSN_VTU_OUTPUT_EVOL_INDICES[bssn::BSSN_NUM_EVOL_VARS_VTU_OUTPUT-1]<<"]"<<NRM<<std::endl;

            sout<<YLW<<"\tBSSN_NUM_CONST_VARS_VTU_OUTPUT :"<<bssn::BSSN_NUM_CONST_VARS_VTU_OUTPUT<<NRM<<std::endl;
            ...
            sout<<YLW<<"\tTPID_PAR_P_PLUS : ( "<<TPID::par_P_plus[0]<<", "<<TPID::par_P_plus[1]<<", "<<TPID::par_P_plus[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_PAR_P_MINUS : ( "<<TPID::par_P_minus[0]<<", "<<TPID::par_P_minus[1]<<", "<<TPID::par_P_minus[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_PAR_S_PLUS : ( "<<TPID::par_S_plus[0]<<", "<<TPID::par_S_plus[1]<<", "<<TPID::par_S_plus[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_PAR_S_MINUS : ( "<<TPID::par_S_minus[0]<<", "<<TPID::par_S_minus[1]<<", "<<TPID::par_S_minus[2]<<" )"<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_CENTER_OFFSET : ( "<<TPID::center_offset[0]<<", "<<TPID::center_offset[1]<<", "<<TPID::center_offset[2]<<" )"<<NRM<<std::endl;

            sout<<YLW<<"\tTPID_INITIAL_LAPSE_PSI_EXPONENT :"<<TPID::initial_lapse_psi_exponent<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_NPOINTS_A :"<<TPID::npoints_A<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_NPOINTS_B :"<<TPID::npoints_B<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_NPOINTS_PHI :"<<TPID::npoints_phi<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_GIVE_BARE_MASS :"<<TPID::give_bare_mass<<NRM<<std::endl;
            sout<<YLW<<"\tINITIAL_LAPSE :"<<TPID::initial_lapse<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_SOLVE_MOMENTUM_CONSTRAINT :"<<TPID::solve_momentum_constraint<<NRM<<std::endl;
            sout<<YLW<<"\tTPID_GRID_SETUP_METHOD :"<<TPID::grid_setup_method<<NRM<<std::endl;
    ...
    *---------------------------------------------------------------------*/ 
    ...
    double computeWTolDCoords(double x,double y,double z, double* hx)
    {
        ...
        if (bssn::BSSN_USE_WAVELET_TOL_FUNCTION == 1) {
            ...
            const double dbh1 = (grid_p-bssn::BSSN_BH_LOC[1]).abs();
                ...
                {
                    if((dbh0 > R_MAX) && (dbh1 > R_MAX)) // no need to check individual points in the element
                        return tolMax;
                    else
                    {
                        for(unsigned int k=0; k < (eleOrder+1); k++)
                        ...
                        if(dbh0 < dbh1)
                            return std::min( tolMax, std::max( tolMin, (tolMax - tolMin)/(R_MAX - R0)*(dbh0 - R0) + tolMin));
                        else
                            return std::min( tolMax, std::max( tolMin, (tolMax - tolMin)/(R_MAX - R1)*(dbh1 - R1) + tolMin));
    ...
    void computeBHLocations(const ot::Mesh* pMesh, const Point* in, Point* out, double** zipVars, double dt)
    {
        ...
        if(pMesh->isActive())
        {
            ...
            ot::da::interpolateToCoords(pMesh,zipVars[VAR::U_BETA0],bh_pts,6,grid_limits,domain_limits,beta0,validIndex_beta0);
            ot::da::interpolateToCoords(pMesh,zipVars[VAR::U_BETA1],bh_pts,6,grid_limits,domain_limits,beta1,validIndex_beta1);
            ot::da::interpolateToCoords(pMesh,zipVars[VAR::U_BETA2],bh_pts,6,grid_limits,domain_limits,beta2,validIndex_beta2);
        ...
        for(unsigned int ind = 0; ind < validIndex_beta0.size(); ind++)
        {
            ...
            //std::cout<<"rank: "<<gRank<<"beta["<<(validIndex_beta0[ind]*3)<<"]: ( "<<beta3vec[validIndex_beta0[ind]*3 + 0]<<", "<<beta3vec[validIndex_beta0[ind]*3 + 1]<<", "<<beta3vec[validIndex_beta0[ind]*3 + 2]<<")"<<std::endl;
            red_ranks[validIndex_beta2[ind]] =gRank;
        }

        par::Mpi_Allreduce(red_ranks,red_ranks_g,2,MPI_MAX,pMesh->getMPIGlobalCommunicator());
        MPI_Bcast(&beta3vec[0],3,MPI_DOUBLE,red_ranks_g[0],pMesh->getMPIGlobalCommunicator());
        MPI_Bcast(&beta3vec[3],3,MPI_DOUBLE,red_ranks_g[1],pMesh->getMPIGlobalCommunicator());
        
        // if(!pMesh->getMPIRankGlobal())
        //     std::cout<<"beta bh0: ( "<<beta3vec[0]<<", "<<beta3vec[1]<<", "<<beta3vec[2]<<") :  beta 1 ( "<<beta3vec[3]<<", "<<beta3vec[4]<<", "<<beta3vec[5]<<") "<<std::endl;
