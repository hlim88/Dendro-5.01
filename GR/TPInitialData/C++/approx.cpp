...
void init_data_puncture(const double *coord, double *u)
{
  ...
  double mass1 = TPID::par_m_plus;
  double bh1x = TPID::par_b;
  double bh1y =  0.0;
  double bh1z =  0.0;
  double vp1[3];
  vp1[0] = TPID::par_P_plus[0];
  vp1[1] = TPID::par_P_plus[1];
  vp1[2] = TPID::par_P_plus[2];
  double vp1tot = sqrt( vp1[0]*vp1[0] + vp1[1]*vp1[1] + vp1[2]*vp1[2] );
  double vs1[3];
  vs1[0] = TPID::par_S_plus[0];
  vs1[1] = TPID::par_S_plus[1];
  vs1[2] = TPID::par_S_plus[2];
  double spin1 = sqrt( vs1[0]*vs1[0] + vs1[1]*vs1[1] + vs1[2]*vs1[2] );

  // bh 2
  double mass2 = TPID::par_m_minus;
  double bh2x =  -TPID::par_b;
  double bh2y =  0.0;
  double bh2z =  0.0;
  double vp2[3];
  vp2[0] = TPID::par_P_minus[0];
  vp2[1] = TPID::par_P_minus[1];
  vp2[2] = TPID::par_P_minus[2];
  double vp2tot = sqrt( vp2[0]*vp2[0] + vp2[1]*vp2[1] + vp2[2]*vp2[2] );
  double vs2[3];
  vs2[0] = TPID::par_S_minus[0];
  vs2[1] = TPID::par_S_minus[1];
  vs2[2] = TPID::par_S_minus[2];
