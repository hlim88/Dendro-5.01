...
*/
  ...
  antisymmetric_lapse = (initial_lapse == LAPSE_ANTISYMMETRIC) ? 1 : 0;
  averaged_lapse = (initial_lapse == LAPSE_AVERAGED) ? 1 : 0;
	pmn_lapse = (initial_lapse == LAPSE_PSIN) ? 1 : 0 ;
  if (pmn_lapse)
		printf("Setting initial lapse to psi^%f profile.\n",
               initial_lapse_psi_exponent);
  brownsville_lapse = (initial_lapse == LAPSE_BROWNSVILLE) ? 1 : 0;
  ...
  for (int k = 0; k < nz; k++)
  {
    for (int j = 0; j < ny; j++)
    {
      for (int i = 0; i < nx; i++)
      {
        ...
        CCTK_REAL xx, yy, zz;
        xx = cctkGH->x[ind] - center_offset[0];
        yy = cctkGH->y[ind] - center_offset[1];
        zz = cctkGH->z[ind] - center_offset[2];
...
*/
          ...
          if (pmn_lapse)
            cctkGH->alp[ind] = pow(p, initial_lapse_psi_exponent);
          if (brownsville_lapse)
            cctkGH->alp[ind] = 2.0/(1.0+pow(p, initial_lapse_psi_exponent));
