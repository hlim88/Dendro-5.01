...
namespace ot
{
    ...
    template<typename T>
    DA::DA(std::function<void(T,T,T,T*)>func,unsigned int dofSz,MPI_Comm comm,unsigned int order,double interp_tol, unsigned int grainSz,double sfc_tol)
    {
    ...
    template <typename T>
    int DA::createVector(T*& local, bool isElemental, bool isGhosted, unsigned int dof)
    {
    ...
    template<typename T>
    int DA::createVector(std::vector<T>& local, bool isElemental, bool isGhosted, unsigned int dof)
    {
        if(!(m_uiMesh->isActive()))
        {
            local.clear();
    ...
    template <typename T>
    void DA::destroyVector(std::vector<T>& local)
    {
        local.clear();
    }

    template<typename T>
    void DA::nodalVecToGhostedNodal(const T* in, T*& out,bool isAllocated,unsigned int dof)
    {
    ...
    template<typename T>
    void DA::ghostedNodalToNodalVec(const T* gVec,T*& local,bool isAllocated,unsigned int dof)
    {
        if(!(m_uiMesh->isActive()))
            return;
    ...
    template <typename T>
    void DA::getElementNodalValues(const T*in, T* eleVecOut,unsigned int eleID,unsigned int dof)
    {
    ...
    template <typename T>
    void DA::eleVecToVecAccumilation(T*out, const T* eleVecIn,unsigned int eleID,unsigned int dof)
    {
        assert((eleID>=m_uiMesh->getElementLocalBegin()) && (eleID<m_uiMesh->getElementLocalEnd()));
        const unsigned int nPe=m_uiMesh->getNumNodesPerElement();
    ...
    template<typename T>
    T* DA::getVecPointerToDof(T* in ,unsigned int dofInex, bool isElemental,bool isGhosted)
    {
    ...
    template<typename T>
    void DA::copyVector(T* dest,const T* source,bool isElemental,bool isGhosted)
    {
        if(!(m_uiMesh->isActive()))
            return ;
    ...
    template<typename T>
    void DA::copyVectors(T* dest,const T* source,bool isElemental,bool isGhosted,unsigned int dof)
    {
        if(!(m_uiMesh->isActive()))
            return ;
    ...
    void DA::intergridTransfer(const T* varIn, T*& varOut,const ot::DA* newDA, bool isElemental, bool isGhosted, unsigned int dof)
    {
        ...
        const ot::Mesh* oldMesh=this->getMesh();
        ...
        if(!isGhosted)
        {
           ...
           for(unsigned int var=0;var<dof;var++)
               newDA->ghostedNodalToNodalVec((const T*)vIn[var],varOut+var*zipSzNew,true,1);
    ...
    template<typename T>
    bool DA::getFaceNeighborValues(unsigned int eleID, const T* in, T* out, T* coords, unsigned int * neighID, unsigned int face)
    {
        if(!m_uiMesh->isActive())
            return false;

        bool state=m_uiMesh->getFaceNeighborValues(eleID,in,out,coords,neighID,face);
