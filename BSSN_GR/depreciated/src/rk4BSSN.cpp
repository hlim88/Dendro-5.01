...
namespace ode
{
    namespace solver
    {
        ...
        RK4_BSSN::RK4_BSSN(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh): RK(pMesh,pTBegin,pTEnd,pTh)
        {
            ...
            m_uiStage=new double**[bssn::BSSN_RK4_STAGES];
        ...
        void RK4_BSSN::applyInitialConditions(double** zipIn)
        {
        ...
        void RK4_BSSN::initialGridConverge()
        {
        ...
        void RK4_BSSN::reallocateMPIResources()
        {
            for(unsigned int index=0;index<bssn::BSSN_ASYNC_COMM_K;index++)
            {
                delete [] m_uiSendNodeBuf[index];
        ...
        void RK4_BSSN::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices)
        {
            ...
            std::function<void(double,double,double,double*)> u_x_t=[](double x,double y,double z,double*var){bssn::KerrSchildData(x,y,z,var);};
                    ...
                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                        for(unsigned int i=0;i<(eleOrder+1);i++)
                        {
                            ...
                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                            {
                                ...
                                u_x_t((double)x,(double)y,(double)z,var);
                                diffVec[nodeLookUp_CG]=var[bssn::VAR::U_CHI]-evolZipVarIn[bssn::VAR::U_CHI][nodeLookUp_CG];
                                chiAnalytical[nodeLookUp_CG]=var[bssn::VAR::U_CHI];
        ...
        void RK4_BSSN::performGhostExchangeVars(double** zipIn)
        {
        ...
        void RK4_BSSN::intergridTransferVars(double**& zipIn, const ot::Mesh* pnewMesh)
        {
        ...
        void RK4_BSSN::unzipVars(double** zipIn , double** uzipOut)
        {
        ...
        void RK4_BSSN::unzipVars_async(double ** zipIn , double **uzipOut)
        {
        ...
        void RK4_BSSN::zipVars(double** uzipIn , double** zipOut)
        {
        ...
        void RK4_BSSN::applyBoundaryConditions()
        {

        }

        void RK4_BSSN::performSingleIteration()
        {
            ...
            do
            {
                ...
                for(unsigned int index=0;index<bssn::BSSN_NUM_VARS;index++)
                    ...
                    /*for(unsigned int index=0;index<bssn::BSSN_NUM_VARS;index++)
                        for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                            m_uiStage[(bssn::BSSN_RK4_STAGES-1)][index][node]*=m_uiT_h;*/
...
//                     io::vtk::waveletsToVTU(m_uiMesh,(const double **)m_uiPrevVar,(const double**)m_uiUnzipVar,refineVarIds,refineNumVars,(const char*)fPrefix);
        ...
        void RK4_BSSN::storeCheckPoint(const char * fNamePrefix)
        {
        ...
        void RK4_BSSN::restoreCheckPoint(const char * fNamePrefix,MPI_Comm comm)
        {
